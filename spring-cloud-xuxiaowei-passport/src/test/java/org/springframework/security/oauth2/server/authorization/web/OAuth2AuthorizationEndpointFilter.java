...
public final class OAuth2AuthorizationEndpointFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if (!authenticationResult.isAuthenticated()) {
				...
				// pass through the chain with the expectation that the authentication
				// process
	...
	private void sendAuthorizationResponse(HttpServletRequest request, HttpServletResponse response,
		...
		String redirectUri = uriBuilder.build(true).toUriString(); // build(true) ->
																	// Components are
																	// explicitly encoded
		this.redirectStrategy.sendRedirect(request, response, redirectUri);
	}
	...
	private static class DefaultConsentPage {
		...
		private static void displayConsent(HttpServletRequest request, HttpServletResponse response, String clientId,
				Authentication principal, Set<String> requestedScopes, Set<String> authorizedScopes, String state)
				throws IOException {

			String consentPage = generateConsentPage(request, clientId, principal, requestedScopes, authorizedScopes,
					state);
		...
		private static String generateConsentPage(HttpServletRequest request, String clientId, Authentication principal,
			...
			for (String scope : requestedScopes) {
				...
				else if (!scope.equals(OidcScopes.OPENID)) { // openid scope does not
																// require consent
			...
			// builder.append(" <link rel=\"stylesheet\"
			...
			builder.append("            <p><span class=\"font-weight-bold text-primary\">" + clientId
					+ "</span> wants to access your account <span class=\"font-weight-bold\">" + principal.getName()
					+ "</span></p>");
			builder.append("        </div>");
			builder.append("    </div>");
			...
			for (String scope : scopesToAuthorize) {
				...
				builder.append(
						"                    <input class=\"form-check-input\" type=\"checkbox\" name=\"scope\" value=\""
								+ scope + "\" id=\"" + scope + "\">");
				builder.append("                    <label class=\"form-check-label\" for=\"" + scope + "\">" + scope
						+ "</label>");
				builder.append("                </div>");
			}
			...
			if (!scopesPreviouslyAuthorized.isEmpty()) {
				...
				for (String scope : scopesPreviouslyAuthorized) {
					...
					builder.append("                    <label class=\"form-check-label\" for=\"" + scope + "\">"
							+ scope + "</label>");
					builder.append("                </div>");
				}
			}
