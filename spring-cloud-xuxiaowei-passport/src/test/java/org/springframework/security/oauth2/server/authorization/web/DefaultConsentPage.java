...
class DefaultConsentPage {
	...
	private static String generateConsentPage(HttpServletRequest request, String clientId, Authentication principal,
		...
		for (String scope : requestedScopes) {
			...
			else if (!scope.equals(OidcScopes.OPENID)) { // openid scope does not require
															// consent
		...
		if (userCode != null) {
			...
			builder.append("        <div class=\"col text-center\">");
			builder.append(
					"            <p class=\"alert alert-warning\">You have provided the code <span class=\"font-weight-bold\">"
							+ userCode + "</span>. Verify that this code matches what is shown on your device.</p>");
			builder.append("        </div>");
			builder.append("    </div>");
		}
		...
		builder.append("        <div class=\"col text-center\">");
		builder.append(
				"            <form name=\"consent_form\" method=\"post\" action=\"" + request.getRequestURI() + "\">");
		...
		if (!scopesPreviouslyAuthorized.isEmpty()) {
			builder
				.append("                <p>You have already granted the following permissions to the above app:</p>");
